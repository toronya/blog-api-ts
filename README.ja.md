...
APIのテストは [Vitest](https://vitest.dev/) を使用して実装されています。テスト実行時はインメモリ SQLite を使用するため、実際のデータベースファイルには影響しません。
...
`src/routes/blogs.test.ts` に以下のエンドポイントのテスト（計21件）が含まれています：

| エンドポイント | テスト内容 |
|---|---|
| `GET /health` | 200 OK を返す |
| `GET /blogs` | 空配列・複数件・DESC ソート |
| `GET /blogs/search` | `q` 必須バリデーション・タイトル検索・本文検索・ヒットなし |
| `GET /blogs/:id` | 無効 id (400)・404・正常取得 |
| `POST /blogs` | バリデーション (400)・201 作成・トリム処理 |
| `PUT /blogs/:id` | 無効 id・バリデーション・404・正常更新 |
| `DELETE /blogs/:id` | 無効 id・404・204 削除後に GET が 404 |
